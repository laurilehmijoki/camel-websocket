...
public class WebsocketProducer extends DefaultProducer {
	...
	public void process(Exchange exchange) throws Exception {
		...
		if (isSendToAllSet(in)) {
			...
			String connectionKey = in.getHeader(
			...
			if (connectionKey != null) {
				...
				throw new IllegalArgumentException(
						"Failed to send message to single connection; connetion key not set.");
			}
		}
	}
			...
			try {
				...
				if (exception == null) {
					exception = new Exception(
							"Failed to deliver message to one or more recipients.",
							e);
				}
			}
		}
	...
	void sendMessage(DefaultWebsocket websocket, String message)
			throws IOException {
